...
local function blowup()
    ...
    for k=1,8 do
        ...
        Sprite(sht2,seqflame):addto(grp):set{scale=s1,dx=dx1, dy=dy1}:play()
        local dx2, dy2 = rand(-15,15)/10, rand(-15,15)/10
        Sprite(sht3,seqflame):addto(grp):setdxdy(dx2,dy2):play()
    end
   
    -- flash effect
    local flash = Image(u'flash.png'):addto(grp)
    flash:setrot(rand(-30,30)):setdxscale(0.4):removeafter(100)
    flash.update = updflash

    -- background light effect
    local hit = Sprite(shthit,seqhit):addto(grp)
    hit:set{rot=rand(360), scale=2.2}
